...
//        initialCacheConfiguration.put(CacheConstant.REPORT_CONTENT_CACHE_KEY, getDefaultSimpleConfiguration(classLoader).entryTtl(Duration.ofDays(5)));
        ...
        initialCacheConfiguration.put(CacheConstant.CHECK_PROCESS_LOG_CACHE_KEY, getDefaultSimpleConfiguration(classLoader).entryTtl(Duration.ofDays(15)));
        // 检测数据的缓存不需要保留很久
        initialCacheConfiguration.put(CacheConstant.CHECK_TASK_CONTENT_CACHE_KEY, getDefaultSimpleConfiguration(classLoader).entryTtl(Duration.ofDays(3)));
        initialCacheConfiguration.put(CacheConstant.CHECK_TASK_PARA_CACHE_KEY, getDefaultSimpleConfiguration(classLoader).entryTtl(Duration.ofDays(3)));
