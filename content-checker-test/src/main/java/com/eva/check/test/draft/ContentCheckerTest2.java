 ...
 * @author zzz
 */
public class ContentCheckerTest2 {
    private static final int BIT_NUM = 64;
/*
*
* simhash3:-4873634358443736615
simhash4:-243950981421284941
calSimilar:75.0
*
*
* */
...
5、批量化处理*/
        ...
        List<String> sourceKeywordList2 = sourceKeywordList.stream().filter(k -> k.length() > 1).filter(StopWordRemover::isNotStopWord).toList();
        List<String> targetKeywordList2 = targetKeywordList.stream().filter(k -> k.length() > 1).filter(StopWordRemover::isNotStopWord).toList();
        ...
        long simhash = SimHashUtil.hash(sourceKeywordList2);
        long simhash2 = SimHashUtil.hash(targetKeywordList2);
